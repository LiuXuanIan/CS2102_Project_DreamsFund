		...
		<div class="container">
			<div class="row animate-box">
				<div class="col-md-8 col-md-offset-2 text-center fh5co-heading">
					<form action="editUserProfile.php" method="POST" autocomplete="off">
					...
					<?php
						$accountEmail = $_GET['userEmail'];

						$db = pg_connect("host=localhost port=5432 dbname=CrowdFunding user=postgres password=1Dcyq7!!");

						if(isset($_POST['Save'])) {
							$result = pg_query($db, "UPDATE users SET first_name = '$_POST[first_name]', last_name = '$_POST[last_name]', location = '$_POST[location]' WHERE email = '$_GET[userEmail]'");
							if (!$result) {
					            echo '<meta http-equiv="refresh" content="0; URL= editUserProfileFailed.php'.'?'.'userEmail='.$accountEmail.'" />';
					        } else {
					            echo '<meta http-equiv="refresh" content="0; URL= editUserProfileSuccess.php'.'?'.'userEmail='.$accountEmail.'" />';
					        }
						}
					?>
